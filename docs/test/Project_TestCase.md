...
### 2.2 データアクセス層 (MasterDataAccess.ts)
...
### 2.3 判定エンジン - 欠格要件 (DisqualificationJudge.ts)
...
### 2.5 判定エンジン - 地域要件 (RegionJudge.ts)
...
### 2.6 判定エンジン - 実績要件 (AchievementJudge.ts)
...
### 2.7 判定エンジン - 技術者要件 (EngineerJudge.ts)
